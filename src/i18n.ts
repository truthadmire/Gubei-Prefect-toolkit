export type Lang = "zh" | "en";

const EN_COPY = {
  setup: "Setup",
  result: "Result",
  today: "Today",
  announcementBrief: "Announcement brief",
  announcementTitle: "Announcement title",
  announcementDate: "Announcement date",
  titlePh: "Enter announcement title",
  titleRequired: "Please enter an announcement title.",
  datePh: "Enter date of announcement",
  dateRequired: "Please enter the announcement date.",
  emptyPeople: "Select at least one Prefect.",
  emptyRooms: "Select at least one form.",
  generationInfeasible: "These selections cannot satisfy the room-coverage rules. Adjust the people or forms.",
  confirmTitle: "Review exclusions",
  confirmBody: "Some people or forms are not selected. This rota will skip the following items.",
  confirmPeople: "Deselected Prefects",
  confirmForms: "Deselected Forms",
  confirmBack: "Go Back",
  confirmContinue: "Continue Anyway",
  date: "Date",
  restoreTitle: "Previous work",
  status: (peo: number, rooms: number, pairs: number, can: number) =>
    `Prefects: ${peo}, Rooms: ${rooms} (need ${pairs} double-duty; available: ${can})`,
  peopleSel: "Prefects",
  peopleHint: "Choose who is on duty and who may cover a second room.",
  formSel: "Forms",
  formsHint: "Select individual forms or use a whole-grade checkbox.",
  selectedLabel: "Selected",
  ddLabel: "Double-duty",
  personSelectedA11y: (name: string) => `${name} Selected`,
  personDoubleA11y: (name: string) => `${name} Double-duty`,
  gradeAllA11y: (grade: string) => `${grade} All forms`,
  summaryTitle: "Live summary",
  activePeopleLabel: "Selected Prefects",
  enabledRoomsLabel: "Selected rooms",
  requiredDoubleLabel: "Required double-duty",
  availableDoubleLabel: "Available double-duty",
  feasible: "Current selection is feasible.",
  infeasible: "Current selection is not feasible.",
  generate: "Generate rota",
  next: "Next",
  back: "Back",
  exportShare: "Share (Mobile/AirDrop)",
  download: "Download JPG",
  downloadExcel: "Download Excel",
  copyJPG: "Copy Image",
  copyJPGOk: "Image copied to clipboard",
  jpgFail: "Could not generate the image. Please try again.",
  excelOk: "Excel table downloaded",
  excelFail: "Could not generate the Excel table. Please try again.",
  shareFail: "Sharing not supported on this device, image copied instead.",
  shareUnavailable: "Sharing is unavailable on this device.",
  assignmentSheet: "Assignment sheet",
  actionsLabel: "Rota actions",
  unassigned: "Unassigned",
  historyTitle: "History",
  historyAutomatic: "The newest shared record is applied automatically to reduce repeated assignments.",
  historySearch: "Search history",
  historyFilter: "History source",
  historyAll: "All",
  historyShared: "Shared",
  historyDevice: "This device",
  historyLoadMore: "Load more",
  historyLoading: "Loading…",
  historyEmpty: "No matching history",
  historyOffline: "Shared history is unavailable. Offline mode is active; new rotas will save on this device and sync automatically when the connection returns.",
  historyRetry: "Retry",
  historyLocal: "On this device",
  historyQueued: "Waiting to share",
  historySharedStatus: "Shared publicly",
  historyFailed: "Retry pending",
  historySelect: "Select generation history",
  historyUse: "Load",
  historyClear: "Clear this device",
  historyLoaded: "History applied to the next rota",
  historyCleared: "Local history cleared",
  colFormRoom: "Class + Room",
  colNameDept: "Name + Department",
  gradeTitle: (grade: number) => (grade === 999 ? "Other Forms" : `Grade ${grade}`),
  gradeToggle: "All",
  doubleDutyBadge: "Double",
  dragHint: "Drag a person to another row to swap positions. On phone, tap two rows to swap.",
  dragDoubleBlocked: (name: string) => `${name} is not enabled for double duty and cannot move to a double-duty slot.`,
  dragDoubleBlockedGeneric: "This swap would move someone without double-duty permission into a paired slot.",
  dragHepburnBlocked: "Hepburn He cannot be assigned to Grade 12 forms.",
  dragMissing: "Could not find the rota positions to swap.",
  dragSameSlot: "Choose a different row to swap.",
  dragUpdated: "Rota updated",
  languageLabel: "Language",
  languageZh: "Chinese",
  languageEn: "English",
  ddTooFew: (need: number, have: number) =>
    `Not enough double-duty people: need ${need}, have ${have}. Enable more "Double" or reduce rooms.`,
  loading: "Loading roster...",
  rosterRecoveryTitle: "名单加载失败 / Roster unavailable",
  rosterRecoveryBody: "无法加载 roster.json，请检查文件后重试。 / Could not load roster.json. Check the file and try again.",
  rosterRetry: "重试 / Retry roster",
  noDept: "No Department",
  footer: "Generated via Gubei Prefect Toolkit",
};

export type AppCopy = typeof EN_COPY;

const ZH_COPY: AppCopy = {
  setup: "准备界面",
  result: "成品界面",
  today: "今日",
  announcementBrief: "公告信息",
  announcementTitle: "公告标题 / Announcement title",
  announcementDate: "公告日期 / Announcement date",
  titlePh: "输入公告标题 / Enter announcement title",
  titleRequired: "请输入公告标题",
  datePh: "输入公告日期 / Enter date of announcement",
  dateRequired: "请输入公告日期",
  emptyPeople: "请至少选择一位 Prefect。",
  emptyRooms: "请至少选择一个班级。",
  generationInfeasible: "当前选择无法满足房间覆盖规则，请调整人员或班级。",
  confirmTitle: "确认排除项",
  confirmBody: "有人员或班级未被选中，本次排布将跳过以下项目。",
  confirmPeople: "未选 Prefect",
  confirmForms: "未选班级",
  confirmBack: "返回修改",
  confirmContinue: "仍然继续",
  date: "日期",
  restoreTitle: "上一轮排布",
  status: (peo: number, rooms: number, pairs: number, can: number) =>
    `Prefects: ${peo}，房间: ${rooms}（需 ${pairs} 位双班；可双班: ${can}）`,
  peopleSel: "Prefect 选择",
  peopleHint: "选择本轮值勤人员，并设置可承担双班的人员。",
  formSel: "班级（Form）选择",
  formsHint: "选择单个班级，或使用整级复选框。",
  selectedLabel: "已选择",
  ddLabel: "双班",
  personSelectedA11y: (name: string) => `${name} 已选择`,
  personDoubleA11y: (name: string) => `${name} 双班`,
  gradeAllA11y: (grade: string) => `${grade} 整级`,
  summaryTitle: "实时概览",
  activePeopleLabel: "已选 Prefect",
  enabledRoomsLabel: "已选房间",
  requiredDoubleLabel: "需要双班",
  availableDoubleLabel: "可双班人员",
  feasible: "当前选择可以生成排布。",
  infeasible: "当前选择暂不可生成排布。",
  generate: "生成排布",
  next: "下一步",
  back: "返回",
  exportShare: "分享 (手机/AirDrop)",
  download: "下载图片",
  downloadExcel: "下载 Excel",
  copyJPG: "复制图片",
  copyJPGOk: "图片已复制到剪贴板",
  jpgFail: "图片生成失败，请重试。",
  excelOk: "Excel 表格已下载",
  excelFail: "Excel 表格生成失败，请重试。",
  shareFail: "当前设备不支持直接分享，已自动为您复制图片",
  shareUnavailable: "当前设备无法使用分享功能。",
  assignmentSheet: "值勤排布单",
  actionsLabel: "排布操作",
  unassigned: "未安排",
  historyTitle: "历史记录",
  historyAutomatic: "系统会自动应用最新的共享记录，减少近期重复安排。",
  historySearch: "搜索历史",
  historyFilter: "历史来源",
  historyAll: "全部",
  historyShared: "公开共享",
  historyDevice: "本机",
  historyLoadMore: "加载更多",
  historyLoading: "正在加载…",
  historyEmpty: "没有匹配的历史记录",
  historyOffline: "共享历史暂时不可用。已进入离线模式；新排布会保存在本机，并在网络恢复后自动同步。",
  historyRetry: "重试",
  historyLocal: "仅保存在本机",
  historyQueued: "等待公开同步",
  historySharedStatus: "已公开共享",
  historyFailed: "等待重试",
  historySelect: "选择历史记录",
  historyUse: "载入",
  historyClear: "清除此设备记录",
  historyLoaded: "历史记录已用于下一轮排布",
  historyCleared: "本机历史已清空",
  colFormRoom: "班级 + 房号",
  colNameDept: "姓名 + 部门",
  gradeTitle: (grade: number) => (grade === 999 ? "其他班级" : `${grade} 年级`),
  gradeToggle: "整级",
  doubleDutyBadge: "双班",
  dragHint: "拖动人员到另一行交换位置；手机可点选两行交换。",
  dragDoubleBlocked: (name: string) => `${name} 未开启双班，不能移动到双班位置。`,
  dragDoubleBlockedGeneric: "此交换会将未开启双班的人员移到双班位置。",
  dragHepburnBlocked: "Hepburn He 不能被安排到 12 年级班级。",
  dragMissing: "无法找到要交换的排布位置。",
  dragSameSlot: "请选择另一行进行交换。",
  dragUpdated: "排布已更新",
  languageLabel: "语言/Language",
  languageZh: "中文",
  languageEn: "English",
  ddTooFew: (need: number, have: number) =>
    `可双班人员不足：需要 ${need} 位，当前 ${have} 位。请勾选更多“双班”或减少房间数。`,
  loading: "正在加载名单…",
  rosterRecoveryTitle: "名单加载失败 / Roster unavailable",
  rosterRecoveryBody: "无法加载 roster.json，请检查文件后重试。 / Could not load roster.json. Check the file and try again.",
  rosterRetry: "重试 / Retry roster",
  noDept: "未分类",
  footer: "由 Gubei Prefect Toolkit 生成",
};

export const I18N: Record<Lang, AppCopy> = {
  zh: ZH_COPY,
  en: EN_COPY,
};
